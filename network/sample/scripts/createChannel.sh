...
createChannel() {
	...
	local COUNTER=1
	local bft_true=$1
	infoln "Adding orderer to channel '$CHANNEL_NAME'"
	while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
		sleep $DELAY
		set -x
    . scripts/orderer.sh ${CHANNEL_NAME}> /dev/null 2>&1
	# removed this for now as it is not needed
...
joinChannel() {
  ...
  setGlobals $ORG
	...
	cat log.txt
	verifyResult $res "After $MAX_RETRY attempts, ${ORG}_peer.${ORG} has failed to join channel '$CHANNEL_NAME' "
}
...
# we will have three channels
# 1. C1 - company, suppliera
# 2. C2 - company, supplierb
