...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang" ] ; then
	...
	CC_SRC_PATH="./chaincode/java/stock/build/install/chaincode"

	echo Compiling Java code ...
	pushd ./chaincode/java/stock
	./gradlew installDist
	popd
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -c '{"Args":["getExchangesByOrganization", "company"]}' >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
