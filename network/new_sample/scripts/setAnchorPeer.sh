...
createAnchorPeerUpdate() {
  ...
  if [ $ORG = "company" ]; then
    HOST="peer0.company.enset.com"
    PORT=7051
  elif [ $ORG = "customer" ]; then
    HOST="peer0.customer.enset.com"
    PORT=8051
  elif [ $ORG = "supplier1" ]; then
    HOST="peer0.supplier1.enset.com"
    PORT=9051
  elif [ $ORG = "supplier2" ]; then
    HOST="peer0.supplier2.enset.com"
    PORT=10051
  else
    errorln "${ORG} unknown"
  fi
...
updateAnchorPeer() {
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.enset.com -c $CHANNEL_NAME -f ${TEST_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
