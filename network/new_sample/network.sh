...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    #supplier1
    infoln "Creating supplier1 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-supplier1.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    #supplier2
    infoln "Creating supplier2 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-supplier2.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.company.enset.com company
    peer_cert admin Admin@company.enset.com company

    infoln "Creating customer Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.customer.enset.com customer
    peer_cert admin Admin@customer.enset.com customer

    infoln "Creating supplier1 Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.supplier1.enset.com supplier1
    peer_cert admin Admin@supplier1.enset.com supplier1

    infoln "Creating supplier2 Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.supplier2.enset.com supplier2
    peer_cert admin Admin@supplier2.enset.com supplier2
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/company/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for company, customer, supplier1, supplier2"
