...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.enset.com/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.enset.com/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.enset.com/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.enset.com/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.enset.com/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.enset.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.enset.com-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.enset.com-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.enset.com

    mkdir -p organizations/ordererOrganizations/enset.com/ca
    mkdir -p organizations/ordererOrganizations/enset.com/msp/cacerts
    mkdir -p organizations/ordererOrganizations/enset.com/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/enset.com/orderers
    mkdir -p organizations/ordererOrganizations/enset.com/tlsca

    CERT_DIR=organizations/ordererOrganizations/enset.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.enset.com-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.enset.com-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/enset.com/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/enset.com/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.enset.com-cert.pem"
